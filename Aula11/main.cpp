...
void renderText() {
	...
	char text[64];
	if (picked)
		sprintf(text, "Picked Snowperson number %d", picked);
	else
		strcpy(text, "Nothing selected");

	glColor3ub(131,44,153);
...
void processMouseButtons(int button, int state, int xx, int yy) 
{
	...
	if (state == GLUT_DOWN)  {
		...
		if (button == GLUT_LEFT_BUTTON)
			...
			if (picked)
				printf("Picked Snowperson number %d\n", picked);
			else
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Picking Snowpeople from CG @ DI");
